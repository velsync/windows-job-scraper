...
Reader connections are per-thread and are tracked so :meth:`Database.close`
closes every connection it opened (no leaked readers).
"""
...
class Database:
    """Single-writer database coordinator with tracked per-thread readers.

    The service process owns one write coordinator; readers open separate
    ...
    Every reader connection opened through :meth:`reader` is tracked and closed
    by :meth:`close`, so shutting the service down cannot leak readers against
    a WAL database.
    """
    ...
    def reader(self) -> sqlite3.Connection:
        ...
        The connection is tracked so :meth:`close` can close it.
        """
    ...
    def close(self) -> None:
        """Close every connection this coordinator opened (writer + readers)."""
        with self._lock:
