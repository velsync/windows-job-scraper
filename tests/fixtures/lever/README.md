...
| `postings_list_rejected_member_paged.json` | 3 members: valid, **no `id`**, valid — served with `limit=2` it is a full degraded page followed by a clean short page | S2.6 corrective (ACQ-03, RUN-13): a `PARTIAL` page proposes no continuation cursor, so a later clean page can never launder the broken membership proof into `COMPLETE` |
